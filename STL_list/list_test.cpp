...
void PrintList(const list<T> & lst) {
	int tmp = lst.size();
	if (tmp > 0) {
		typename list<T>::const_iterator i;// typename用来说明list<T>::const_iterator是个类型在VS中不写也可以
		i = lst.begin();
...
//与其他顺序容器不同, list容器只能使用双向迭代器,因此不支持大于/小于比较运算符, []运算符和随机移动
...
int main() 
{
	...
	//lst2.push_back(30); lst2.push_front(30);  //会导致merge崩溃
	lst2.unique(); //删除所有和前一个元素相等的元素 //相隔的相等的元素不会删除
	cout << "6) "; PrintList(lst2); cout << endl;
	lst1.merge(lst2); //合并lst2到lst1并清空lst2   Merge sorted lists，list若无序会导致程序异常终止
